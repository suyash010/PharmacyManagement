<?php


$con=mysqli_connect("localhost","root","","pharmacy");
if (!$con) {
    die("Connection failed: " . mysqli_connect_error());
}
if (isset($_GET['id'])) { // Ensure the parameter matches the one in the button
    $pharmacist_id = $_GET['id'];
    $sql = "DELETE FROM pharmacist WHERE pharmacist_id = '$pharmacist_id'";
    if (mysqli_query($con, $sql)) {
        // Redirect to the pharmacist page with a success status
        header("Location: pharmacist.php?status=success");
        exit();
    } else {
        // Redirect to the pharmacist page with an error status
        header("Location: pharmacist.php?status=error");
        exit();
    }
} else {
    // Redirect to the pharmacist page if no ID is provided
    header("Location: pharmacist.php?status=missing_id");
    exit();
}


?>